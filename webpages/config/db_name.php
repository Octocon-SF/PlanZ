...
if( file_exists( '/var/www/vhosts/octocon.com/participants/private/db_env.php' ) ) {
    require_once( '/var/www/vhosts/octocon.com/participants/private/db_env.php' );
} else {
    echo '<strong>FATAL ERROR: Config file of environment variables not found.</strong><br />';
}

define("CON_NAME", "Octocon 2026");
define("CON_URL", "http://octocon.com");
define("BASE_PATH", "/"); // Base path links will be relative to. Only change if hosting in a subdirectory.
define("BRAINSTORM_EMAIL", "programme@octocon.com");
define("PROGRAM_EMAIL", "programme@octocon.com");
define("CON_NUM_DAYS", 2); // code works for 1 - 8
define("PHP_DEFAULT_TIMEZONE", "EUROPE/DUBLIN"); // must be valid argument to php date_default_timezone_set()  Should correspond with DB configuration
define("DB_DEFAULT_TIMEZONE", "Europe/Dublin"); // must be valid argument to set time_zone,  Should correspond with PHP configuration
define("CON_START_DATIM", "2026-10-03 00:00:00"); // Broadly used.  Must be in mysql format: "YYYY-MM-DD HH:MM:SS" (HH:00-23) HH:MM:SS probably should be 00:00:00
define("DAY_CUTOFF_HOUR", 0); // times before this hour (of 0-23) are considered previous day
        // used for Participant Availability only
define("FIRST_DAY_START_TIME", "08:30"); // next 5 are for grid scheduler
define("OTHER_DAY_STOP_TIME", "23:30");
define("OTHER_DAY_START_TIME", "08:30");
define("LAST_DAY_STOP_TIME", "22:00");
...
define("DISPLAY_24_HOUR_TIME", true); // TRUE: times in 24 hour clock. FALSE: times in 12 hour clock.
define("DURATION_IN_MINUTES", true); // TRUE: in mmm; FALSE: in hh:mm
        // affects session edit/create page only, not reports
define("DEFAULT_DURATION", "60"); // must correspond to DURATION_IN_MINUTES
define("SMTP_QUEUEONLY", true); // TRUE = add to DB queue, schedule /scripts/processEmailQueue.php as a cron job do the send; FALSE send immediately, add to queue only on transport failure
define("SMTP_MAX_MESSAGES", "50"); // Maximum number of messages to send per cron run. Set to 0 for no limit.
...
define("ALLOW_ASSIGN_UNRANKED_PARTICIPANTS", true); // If true, show participants on Assign Participants page if they have not ranked sessions.
define("TITLE_MIN_LENGTH", 10); // Title must be at least this long.
define("TITLE_MAX_LENGTH", 100); // Title must not be longer than this. Note that the current limit of the DB field is 100 characters.
...
define("GRID_START_DATIM", "2026-10-03 08:30:00"); // Used by special grid report
define("GRID_END_DATIM", "2026-10-04 20:00:00"); // Used by special grid report
...
define("PASSWORD_RESET_FROM_EMAIL", "programme@octocon.com"); // From address to be used for password reset emails
define("PASSWORD_RESET_FROM_EMAIL_NAME", "Octocon Programme Team"); // The name accompanying the from address to be used for password reset emails
...
define("USE_REGTYPE_DESCRIPTION", true);
...
define("USE_DAY_JOB", false); // Let participants specify their daytime occupation on Personal Details page.
...
define("LABEL_ETHNICITY", "Ethnicity:"); // Label for ethnicity.
...
define("LABEL_SEXUAL_ORIENTATION", "Romantic/Sexual Orientation:"); // Label for sexual orientation.
...
define("MY_SCHEDULE_SHOW_COMMENTS", false); // Should participant comments be shown on My Schedule?

define("REG_PART_PREFIX", "Z"); // only needed for USE_REG_SYSTEM = FALSE; prefix portion of userid/badgeid before counter; can be empty string for no prefix
...
define("PHOTO_DEFAULT_IMAGE", "user-filled-person-shape-svgrepo-com.svg"); // placeholder image for participants without photo

define("JSON_EXTRACT_DIRECTORY", "/var/www/vhosts/octocon.com/prog/2026/");  // Path to directory where Konopas/ConClár files to be written.
...
define("PHOTO_EXTRACT_LINK_TYPE", "photo"); // Link type to use when exporting photos to KonOpas/ConClár.
...
define("CON_THEME_FAVICON", "themes/octocon/OctoconIcon.ico");
...
define("CON_HEADER_IMG", "themes/octocon/octocon_logo.svg");
...
define("CON_HEADER_IMG_ALT", "Octocon 2026 - The National Irish Science Fiction Convention");
...
define("CON_SUPPORT_EMAIL", "programme@octocon.com"); // From address to be used for password reset emails

define("REPORT_INCLUDE_DIRECTORY", "/var/www/vhosts/octocon.com/planz/reports/");  // outside of web server path, only served by PHP
